...
class PortfolioManager:
    ...
    def _save_portfolios(self):
        ...
        from valutatrade_hub.infra.database import db
        portfolios_data = [portfolio.to_dict()
    ...
    def add_currency_to_portfolio(
            self, user_id: int, currency_code: str) -> Wallet:
    ...
    def deposit_to_wallet(self, user_id: int,
    ...
    def withdraw_from_wallet(
            self, user_id: int, currency_code: str, amount: float) -> bool:
    ...
    def get_all_wallets_info(self, user_id: int) -> Dict[str, float]:
        ...
        portfolio = self.get_user_portfolio(user_id)
        return {currency: wallet.balance for currency,
                wallet in portfolio.wallets.items()}
    ...
    def buy_currency(self, user_id: int, currency_code: str,
        ...
        def _buy_operation():
            try:
                _currency = get_currency(currency_code)
    ...
    def sell_currency(self, user_id: int, currency_code: str,
        ...
        def _sell_operation():
            ...
            if not rate_data:
                raise ApiRequestError(f"не удалось получить курс для {
                                      currency_code}-USD")
    ...
    def __init__(self, data_dir: str = None): # noqa: F811
        from valutatrade_hub.infra.settings import settings
        self.data_dir = data_dir or settings.get('data_dir', 'data')
...
class ExchangeRateService:
    ...
    def get_rate(self, from_currency: str,
                 to_currency: str) -> Optional[Dict[str, Any]]:
            ...
            if "updated_at" in rate_data:
                try:
                    ...
                    if (datetime.datetime.now() -
                            updated_at).total_seconds() < self.rates_ttl:
                        return rate_data
    ...
    def _update_rate(self, from_currency: str,
                       to_currency: str) -> Optional[Dict[str, Any]]:
    ...
    def _get_stub_rate(self, from_currency: str,
                       to_currency: str) -> Optional[Dict[str, Any]]:
        stub_rates = {
